...
module.exports = {
  getFreeTime: async (req, res, next) => {
    ...
    const dateFrom = await new Date(
      new Date(date_from).getTime() - 60 * 60 * 3 * 1000
    );
    const dateUntil = await new Date(
      new Date(date_until).getTime() - 60 * 60 * 3 * 1000
    );
  ...
  getSmartTime: async (req, res, next) => {
    ...
    const smartData = await smart(
      ...
      timeScope,
      true
    ); // Edit it later to add extra options
    if (smartData.error)
      return res.status(404).json({ error: smartData.error });
