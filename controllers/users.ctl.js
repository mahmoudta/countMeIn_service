...
const { timenow } = require('./algs/free-alg');
	...
	getPannel                : async (req, res, next) => {
		...
		if (!isEmpty(appointments)) {
			appointments.map((appointment, i) => {
				All = i;
				if (
					appointment.time.start._hour < Hours ||
					(appointment.time.start._hour == Hours && appointment.time.start._minute < Minutes)
				) {
					Done = Done + 1;
				} else {
